...
bool NodeCostSort::operator ()(node_info *left, node_info *right) const
  {
  double left_perc = max(static_cast<double>(p_procs)/left->get_proc_total(),static_cast<double>(p_mem)/left->get_mem_total());
  double right_perc = max(static_cast<double>(p_procs)/right->get_proc_total(),static_cast<double>(p_mem)/right->get_mem_total());
