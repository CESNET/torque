...
%takhle se udela pozadi pres cely poster
...
%metacentrovy styl (oranzova a sediva z loga)
\def\vyrazne{orange1}
\def\ramecek{mgray}
...
{\vskip.2ex \color{\vyrazne}\textbf{\large #1}\par}
#2
\bigskip
\end{beamercolorbox}
}

\long\def\mygreylbox#1#2{
\begin{beamercolorbox}{greylbox}
\advance\baselineskip.5ex
\leftskip=.5em
\rightskip=.5em
\parskip 1ex plus .5ex minus .5ex
{#1 \textbf{#2\phantom{\raisebox{1ex}{(}}}}
\bigskip
\end{beamercolorbox}
}
...
\institute{CESNET and Masaryk University, Czech Republic}
...
\mysect{Magrathea architecture}
\mygreylbox{\large}{Designed to allow Grid job scheduling systems to deal with several virtual machines running
...
Design Requirements:
\begin{itemize}
\item More active (i.e., running) virtual machines than
...
\item Minimal changes or modifications of the resource
...
\item Independence on particular VM implementation (Xen and VServer in current
implementation)
\end{itemize}
...
\mygreylbox{\Large}{Supported scenarios}
...
\begin{itemize}
\item respect Magrathea status -- submit only to domains
with Magrathea status \emph{free}, \emph{running},
\emph{occupied-would-preempt}, and \emph{running-preemptible}
\item sort nodes using Magrathea status (non-preemption first)
\end{itemize}

\bigskip
\textbf{PBS server} modified to:
\begin{itemize}
\item dedicate queue for high priority jobs
...
\bigskip
Magrathea status also contains number of CPUs used by the virtual machine 
...
%takhle by se udelalo podekovani pod carou
